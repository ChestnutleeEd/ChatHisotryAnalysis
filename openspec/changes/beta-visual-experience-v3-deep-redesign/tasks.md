...
- [ ] 1.1 [Luna Max] Reconfirm the frozen functional contract with synthetic fixtures: report facts, query semantics, Word Cloud worker authority, Canvas resize-only presentation authority, export/share behavior, privacy boundaries, and the B5 Share Preview acceptance suite MUST remain unchanged.
- [ ] 1.2 [Luna Max] Add the V3 token and typography foundation for Organic Annual/Home and Swiss Detailed modes, using only packaged offline-safe font stacks and a specimen page or harness that proves weights, numerals, Chinese fallback, and dense-label legibility.
- [ ] 1.3 [Luna Max] Introduce the V3 presentation primitives (`PageShell`, `Scene`, `SceneHeader`, `StoryGrid`, `MetricCard`, `ChartPanel`, `Annotation`, `SectionDivider`, and `ProgressNavigator`) without migrating report content yet; keep their DOM and accessibility contracts explicit.
- [ ] 1.4 [Luna Max] Build the content-driven Annual shell and the compact progress dock, including semantic chapter buttons, range/chapter controls in normal flow, IntersectionObserver-derived active scene state, deterministic restoration, skip-link/focus behavior, and stable scroll offsets.
- [ ] 1.5 [Luna Max] Add the V3 motion primitives and reduced-motion final-state behavior using the specified duration/easing tiers; prohibit layout-affecting animation and default-hidden content that depends on JavaScript to become readable.
- [ ] 1.6 [Luna Max] Add per-scene and per-cell geometry diagnostics for tail space, occupancy, stretch, pairing ratios, anchor position, horizontal overflow, and the explicit opening/closing allowlist; produce machine-readable evidence from synthetic data at 1440, 1180, 760, 380, and 200 CSS pixels.
- [ ] 1.7 [Luna Max] Migrate only the shell/navigation consumers, consolidate their CSS authority into the new layer boundaries, and delete superseded selector blocks one explicit file at a time without altering report semantics.
- [ ] 1.8 [Luna Max] Run focused unit/browser/a11y tests, capture the required synthetic screenshots, conduct the V3.1 human visual gate, and record evidence for Organic/Swiss differentiation, keyboard navigation, reduced motion, geometry, and packaged offline behavior.
- [ ] 1.9 [Sol xHigh — STOP GATE] Review the complete V3.1 diff against this proposal/design/spec, accept only bounded visual-foundation findings, run both strict OpenSpec validations plus `git diff --check`, then stage only the V3.1 implementation/evidence paths, commit, push, and stop for approval before V3.2.
