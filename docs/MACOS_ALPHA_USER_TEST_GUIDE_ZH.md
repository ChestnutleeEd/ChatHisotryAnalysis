...
如果数据交接校验失败，主错误码保持为 `DATASET_HANDOFF_INVALID`，旁边的原因码只会是
稳定的 `HANDOFF_*` 诊断（例如缺少 chunk、计数不一致或 hash 不一致）。源文件本身的
问题使用单独的 `SOURCE_*` 诊断。可先点击重试；若源选择或会话已经过期，请重新选择
年度源后再开始分析。不要把错误截图中的路径、正文或原始 JSON 一并发送。
